...
METRIC_CONFIGS = {
    "fidelity_plus": {"title": "Fidelity Plus", "color": COLOR_PLUS},
    "fidelity_minus": {"title": "Fidelity Minus", "color": COLOR_MINUS},
}
...
def process_csv(csv_path: Path, grid_points: int) -> None:
    ...
    if df.empty:
        print(f"[skip] Empty fidelity CSV skipped: {csv_path}")
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate KDEs and divergence metrics for fidelity analytics.")
    parser.add_argument(
        "--base-dir",
        type=Path,
        default=BASE_INPUT_ROOT,
        help="Directory containing per-dataset fidelity CSV files.",
    )
    parser.add_argument(
        "--grid-points",
        type=int,
    ...
    for csv_path in tqdm(csv_paths, desc="Generating fidelity KDEs"):
        try:
