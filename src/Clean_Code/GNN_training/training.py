...
class CachedGraphDataset(Dataset):
    ...
    def _load_file(self, path: str) -> Any:
        ...
        cached_batch = self.batch_cache.get(path)
        if cached_batch is not None:
            return cached_batch
        
        batch = self._load_file_direct(path)
        self.batch_cache.put(path, batch)
        return batch
...
class GNNTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if config.get('val_data_dir'):
            ...
            print("\n[2/3] No validation data provided, will skip validation")
    ...
    def train_epoch(self):
        """Train for one epoch with memory monitoring and gradient clipping."""
        self.model.train()
        total_loss = 0
        total_correct = 0
        total_samples = 0
            ...
            try:
                ...
                if (batch_idx + 1) % 10 == 0:
                    ...
                    print(f"\nBatch {batch_idx+1}/{len(self.train_loader)} - "
                          f"Loss: {loss.item():.4f} - "
                          f"Acc: {correct/len(batch.y):.4f} - "
                          f"Time: {batch_time:.3f}s - "
                          f"CPU: {mem['rss_gb']:.2f}GB - "
                          f"GPU: {gpu_mem:.2f}MB")
                
            except RuntimeError as e:
                if 'out of memory' in str(e).lower():
                    print(f"\nOOM on batch {batch_idx+1}, skipping...")
        ...
        print(f"Epoch complete - Avg Loss: {avg_loss:.4f}, Acc: {accuracy:.4f}")
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc='Validation'):
                ...
                total_loss += loss.item() * len(batch.y)  # Weight by batch size
                pred = out.argmax(dim=1)
    ...
    def train(self):
        ...
        for epoch in range(self.config['epochs']):
            ...
            if self.scheduler:
                if isinstance(self.scheduler, ReduceLROnPlateau) and val_results is not None:
                    # Only use validation loss for ReduceLROnPlateau if validation data is available
                    self.scheduler.step(val_results[0] if val_results else train_loss)
                else:
                    self.scheduler.step()
            ...
            if val_results is not None:
                ...
                # Early stopping and model saving based on validation
                ...
                # If no validation data, just save the model every epoch
                print(f"Epoch {epoch+1}/{self.config['epochs']}")
                print(f"Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f}")
                
                # Save model checkpoint (no validation, so we save every epoch)
                best_model_path = os.path.join(output_dir, f'model_epoch_{epoch+1}.pth')
                self.save_model(best_model_path, epoch, train_loss, train_acc)
