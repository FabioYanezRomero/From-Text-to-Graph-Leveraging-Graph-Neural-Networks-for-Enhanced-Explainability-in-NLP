...
RUN grep -v -E 'torch|pytorch_geometric|pyg_lib|torch_scatter|torch_sparse|torch_cluster|torch_spline_conv' /tmp/requirements.txt > /tmp/requirements-subgraphx.txt && \
