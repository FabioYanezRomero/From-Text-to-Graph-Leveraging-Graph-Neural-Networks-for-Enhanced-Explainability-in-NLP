...
class MCTS(object):
    ...
    """
    def __init__(self, X: torch.Tensor, edge_index: torch.Tensor, edge_attr:torch.Tensor, dict_node:dict, num_hops: int,
...
class SubgraphX(object):
    ...
    def __init__(self, model, num_classes: int, device, num_hops: Optional[int] = None, verbose: bool = False,
                 ...
                 expand_atoms=5, high2low=False, local_radius=4, sample_num=10, reward_method='mc_l_shapley',
    ...
    def get_mcts_class(self, x_list, edge_index_list, edge_attr_list, dict_node_list, node_idx_list, score_func: Callable):
        ...
        for x, edge_index, edge_attr, dict_node, node_idx in zip(x_list, edge_index_list, edge_attr_list, dict_node_list, node_idx_list):
    ...
    def explain(self, x_list: list[Tensor], edge_index_list: list[Tensor], 
                ...
                max_nodes: int = 5, node_idx_list = None,
        ...
        if self.explain_graph:                                                                                                  # aquí
            ...
            if not saved_MCTSInfo_list:
                ...
                self.mcts_state_map = self.get_mcts_class(x_list, edge_index_list, edge_attr_list, dict_node_list, node_idx_list, score_func=payoff_func)
                results_list = []
            ...
            self.mcts_state_map = self.get_mcts_class(x_list, edge_index_list, node_idx_list, score_func=None)
            for i in range(len(self.mcts_state_map)):
