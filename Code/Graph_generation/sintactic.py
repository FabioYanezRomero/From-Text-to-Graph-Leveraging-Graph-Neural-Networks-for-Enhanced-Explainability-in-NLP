...
class sintactic_graph_generator():
    ...
    def _parse(self, sentence: list[str]):
        """
        Parse the sentence using the syntactic parser.
        """
        return self.sint.predict(sentence, verbose=False)[0]
    
    def _build_graph(self, tree, id = False):
        ...
        for i in range(len(tree.values[1])):
            graph.add_node(i+1, word=tree.values[1][i])
    ...
    def get_graph(self, sentence: list):
        ...
        graph = self._build_graph(sintactic_tree)
        return graph
