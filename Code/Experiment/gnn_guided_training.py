...
from dataloader import Dataset_GNN, Dataset_RGNN, MyDataLoader, HeteroGeneousDataLoader
...
# Load datasets
if args['module'] in ['GCNConv', 'GATConv', 'GraphConv', 'SAGEConv']:
    train_dataset = Dataset_GNN(root=args['root_train_data_path'], files_path=args['raw_train_data_path'])
    dev_dataset = Dataset_GNN(root=args['root_dev_data_path'], files_path=args['raw_dev_data_path'])
    test_dataset = Dataset_GNN(root=args['root_test_data_path'], files_path=args['raw_test_data_path'])
elif args['module'] in ['RGCNConv', 'RGATConv']:
    mode_flags = {'semantic': False, 'sintactic': False, 'constituency': False}
    for m in args['mode'].split('+'):
        if m in mode_flags:
            mode_flags[m] = True
    train_dataset = Dataset_RGNN(root=args['root_train_data_path'], files_path=args['raw_train_data_path'], **mode_flags)
    dev_dataset = Dataset_RGNN(root=args['root_dev_data_path'], files_path=args['raw_dev_data_path'], **mode_flags)
    test_dataset = Dataset_RGNN(root=args['root_test_data_path'], files_path=args['raw_test_data_path'], **mode_flags)
else:
...
for epoch in range(args['num_epochs']):
    ...
    for file_idx in tqdm(range(len(train_dataset)), desc=f"Training Epoch {epoch}"):
        dataloader = (MyDataLoader(train_dataset[file_idx], batch_size=args['batch_size'], shuffle=True) 
                      if args['module'] in ['GCNConv', 'GATConv', 'GraphConv', 'SAGEConv'] 
                      else HeteroGeneousDataLoader(train_dataset[file_idx], batch_size=args['batch_size'], shuffle=True))
    ...
    with torch.no_grad():
        for file_idx in tqdm(range(len(dev_dataset)), desc=f"Validation Epoch {epoch}"):
            dataloader = (MyDataLoader(dev_dataset[file_idx], batch_size=args['batch_size'], shuffle=False) 
                          if args['module'] in ['GCNConv', 'GATConv', 'GraphConv', 'SAGEConv'] 
                          else HeteroGeneousDataLoader(dev_dataset[file_idx], batch_size=args['batch_size'], shuffle=False))
    ...
    with torch.no_grad():
        for file_idx in tqdm(range(len(test_dataset)), desc=f"Testing Epoch {epoch}"):
            dataloader = (MyDataLoader(test_dataset[file_idx], batch_size=args['batch_size'], shuffle=False) 
                          if args['module'] in ['GCNConv', 'GATConv', 'GraphConv', 'SAGEConv'] 
                          else HeteroGeneousDataLoader(test_dataset[file_idx], batch_size=args['batch_size'], shuffle=False))
